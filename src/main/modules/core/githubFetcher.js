...
async function fetchReleases(moduleId, repo, options = {}) {
    ...
    if (!forceRefresh && releaseCache.isCacheValid(moduleId)) {
    ...
    try {
        ...
        for (const release of rawReleases) {
            ...
            if (assetFilter) {
                matchingAssets = matchingAssets.filter(a => matchGlob(a.name, assetFilter));
            }
        ...
        releaseCache.writeCache(moduleId, mappedReleases);
...
function findAsset(release, assetPattern) {
    ...
    return release.assets.find(a => matchGlob(a.name, assetPattern)) || null;
}
