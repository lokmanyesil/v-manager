...
async function runModuleWizard(event, { manifest, game, version, dllName, downloadUrl, developerPreset, exePath, lang }, shouldAbort) {
    ...
    const logsDir = path.join(app.getPath('userData'), 'logs', `${modId === 'dlssenabler' ? 'dlss-enabler' : modId}-wizard`);
    ...
    const logPath = path.join(logsDir, `${modId === 'dlssenabler' ? 'dlss' : modId}-wizard-${timestamp}.log`);
    ...
    for (let i = 0; i < dllsToTry.length; i++) {
        ...
        // Kopyalanacak diğer dosyalar
        let otherFiles = [];
        try {
            otherFiles = fs.readdirSync(versionDir).filter(f => f.toLowerCase() !== sourceFile.toLowerCase());
        } catch (e) {
